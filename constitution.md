...
1. The package MUST run entirely on the operator's own infrastructure. No component may require a hosted TunnelMind service, a package-author-controlled service, a database server, or a message queue to perform its core function (detect → offer → verify → pass/consequence). Operator-configured third-party payment rails — the operator's own Stripe account, the operator's chosen chain RPC endpoint or x402 facilitator — are permitted for the payment options that require them; every such rail MUST be operator-keyed, adapter-isolated, and off the hot path for non-paying traffic.
...
9. Conduct receipts MUST contain request metadata only (key, path, timestamp, response class, user-agent string). Request/response **bodies MUST never be captured, logged, or transmitted**. A `SPOOF` receipt MUST NOT populate `agent_key` — an unverified claim proves nothing about the key's real holder, and anyone can send garbage signatures naming any key. The claimed key goes in `claimed_key`, explicitly labeled unverified.
...
13. Agent identification uses **Web Bot Auth** (HTTP Message Signatures / RFC 9421 profile). A signature that is *cryptographically wrong* for the claimed key is treated as spoofing (consequence-eligible). A signature that is well-formed and cryptographically valid but *temporally stale* (expired, or `created` outside the clock-skew tolerance) MUST be treated as anonymous, not spoofing — clock skew is a fault, not a forgery, and mazing an honest agent over drift violates Article II. Absence of any signature is treated as anonymous (offer-eligible, never consequence-eligible on first contact).
