...
const candidates = [
  ...
  process.env.TM_SPEC_ROOT ? resolve(process.env.TM_SPEC_ROOT, '.specify/preset.json') : null,
...
if (!found) {
  const out = { name: 'tunnelmind', version: '0.0.0', constitution: null, source: null, note: 'not initialised' };
  console.log(json ? JSON.stringify(out) : `${out.name}@${out.version} (not initialised)`);
  process.exit(0);
}
const p = JSON.parse(readFileSync(found, 'utf8'));
const out = { name: p.name, version: p.version, constitution: p.constitution, source: found };
console.log(json ? JSON.stringify(out) : `${p.name}@${p.version} (constitution ${p.constitution})`);
